...
bool HalfEdgeMesh::AddFace(const std::vector<glm::vec3>& verts) {
    ...
    size_t v1 = AddVertex(verts[0]);
    size_t v2 = AddVertex(verts[1]);
    size_t v3 = AddVertex(verts[2]);
    
    // Add all half-edge pairs
    std::pair<size_t, size_t> hePair1 = AddHalfEdgePair(v1,v2);
    std::pair<size_t, size_t> hePair2 = AddHalfEdgePair(v2,v3);
    std::pair<size_t, size_t> hePair3 = AddHalfEdgePair(v3,v1);
    ...
    // normalized)
    Face theFace;
    theFace.edge = hePair1.first;
    mFaces.push_back(theFace);
    size_t faceIndx = mFaces.size()-1;
    theFace.normal = FaceNormal(faceIndx);
    
    // All half-edges share the same left face (previously added)
    e(hePair1.first).face = e(hePair2.first).face = e(hePair3.first).face = faceIndx;
...
std::vector<size_t> HalfEdgeMesh::FindNeighborFaces(size_t vertexIndex) const {
    ...
    do {
        ...
        foundFaces.push_back(e(temp).face);
        temp = e(e(e(temp).next).next).pair;
...
float HalfEdgeMesh::Area() const {
    ...
    for (int i = 0; i < mFaces.size(); i++) {
        v1 = e(mFaces[i].edge).vert;
        v2 = e(e(mFaces[i].edge).next).vert;
        v3 = e(e(mFaces[i].edge).prev).vert;

        area += glm::length(glm::cross((v(v2).pos-v(v1).pos),(v(v3).pos-v(v1).pos)))/2;
    }
    return area;
}
...
float HalfEdgeMesh::Volume() const {
    ...
    for (int i = 0; i < mFaces.size(); i++) {
        ...
        volume += glm::dot((v(v1).pos + v(v2).pos + v(v3).pos), mFaces[i].normal * area)/9.0f;
